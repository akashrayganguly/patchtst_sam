...
class Model(nn.Module):
    def __init__(self, configs, max_seq_len:Optional[int]=1024, d_k:Optional[int]=None, d_v:Optional[int]=None, norm:str='BatchNorm', attn_dropout:float=0.,
        ...
        # ---- NEW: channel-attention flag ------------------------------
        channel_attn = bool(getattr(configs, 'channel_attn', 0))
        ...
        common_kw = dict(
            ...
            verbose=verbose, channel_attn=channel_attn,
        )

        if self.decomposition:
            self.decomp_module = series_decomp(kernel_size)
            self.model_trend = PatchTST_backbone(**common_kw, **kwargs)
            self.model_res   = PatchTST_backbone(**common_kw, **kwargs)
        else:
            self.model = PatchTST_backbone(**common_kw, **kwargs)
